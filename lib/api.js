...
class PaxApi extends Homey.SimpleClass {
  ...
  constructor(pin, advertisement, homey) {
    ...
    this.pin = pin;
    this._peripheral = this.getPeripheral();
  }
  ...
  async setBoostMode(on, speed, seconds) {
    ...
    try {
      await this.getPeripheral()
        .then((peripheral) => this.auth(peripheral, this.pin))
        .then((peripheral) => peripheral.write(serviceIds.BOOST, characteristics.BOOST, struct.pack('<BHH', on, speed, seconds)));
    } catch (error) {
