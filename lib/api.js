...
const FanSpeed = require('./fanspeed');
...
class PaxApi extends Homey.SimpleClass {
  ...
  /**
   * Set fan speed setting for device.
   */
  async setFanSpeed(humidity = 2250, light = 1625, trickle = 1000) {
    ...
    try {
      ...
      await this.peripheral.write(serviceIds.BOOST, characteristics.LEVEL_OF_FAN_SPEED, struct.pack('<HHH', humidity, light, trickle));
    } catch (error) {
